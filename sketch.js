...
var eaglesGroup, eagleImage, eagleEnd;
var obstaclesGroup, obstacleImg, obstacleEnd;
...
function preload(){
  ...
  missleImage= loadImage ("Images/Missle.png");
  ...
  eagleEnd= loadImage ("Images/Eagle2.png")
  obstacleEnd = loadImage("Images/Enemy jet1.png")
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if (keyDown("j")){
      missle = createSprite(Jet.x , Jet.y)
      missle.addImage(missleImage);
      missle.scale= 0.5;
      missle.depth = Jet.depth-1
      missle.velocityX = 10;
      missles.push(missle);
      
    for(var i = 0; i < missles.length -2; i++) {
      missles [i].destroy()
    }
  }
     for(var i = 0; i < obstaclesGroup.length; i++) {
       var enemy= obstaclesGroup.get(i);
       if (enemy.isTouching(missle)) {
         enemy.destroy();
         points += 50
       }
     }
     for(var i = 0; i < eaglesGroup.length; i++) {
      var enemy= eaglesGroup.get(i);
      if (enemy.isTouching(missle)) {
        enemy.destroy();
        points += 50
      }
    ...
    for(var i=0; i<eaglesGroup.length;i++) {
      var Bird = eaglesGroup.get(i)
     Bird.changeAnimation("eagleEnd")
    }

    for(var i=0; i<obstaclesGroup.length;i++) {
      var jet = obstaclesGroup.get(i)
     jet.changeAnimation("enemyEnd")
    }
  ...
  textSize (25);
  fill ("black")
  text("Distance traveled: "+ score, width-800,50);
  text("Points: "+ points, width-500,100)
}

function spawneagles() {
  //write code here to spawn the eagles
  if (frameCount % 200 === 0) {
   eagle = createSprite(width,random(400,height-50),40,10);
    eagle.addAnimation("eagle",eagleImage);
    eagle.addAnimation("eagleEnd",eagleEnd);
...
function spawnObstacles() {
  if(frameCount % 100 === 0) {
   obstacle = createSprite(width,random(150,height-150),10,40);
    obstacle.addAnimation("enemy jet",obstacleImg);
    obstacle.addAnimation("enemyEnd",obstacleEnd);
